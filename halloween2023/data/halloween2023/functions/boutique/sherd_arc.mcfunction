advancement revoke @s only halloween2023:boutique/sherd_arc
...
execute if score @s h23_monnaie matches 10.. run give @s archer_pottery_sherd{display:{Name:'{"text":"Gravure d\'arc","color":"#B87333","bold":true,"italic":false}',Lore:['{"text":" "}','{"text":"Une gravure utilisée pour","color":"#FAFAFA","italic":false}','{"text":"décorer des pots.","color":"#FAFAFA","italic":false}','{"text":" "}','{"text":"Halloween 2023","color":"#024050","bold":true,"italic":false}']}} 4
execute if score @s h23_monnaie matches 10.. at @s run playsound minecraft:block.amethyst_block.resonate master @s ~ ~ ~
execute if score @s h23_monnaie matches 10.. run tellraw @s [{"text":"Simba Biole","bold":true,"color":"#024050"},{"text":" : Merci d'avoir acheté une ","color":"#C5CDB5","bold":false},{"text":"Gravure d'arc","color":"#C5CDB5"},{"text":" contre 10 pièces temporelles !","color":"#C5CDB5","bold":false}]
